...
from tensorflow.contrib import slim  # 简化版本的tensorflow
import logging
...
class CMAP(object):
    ...
    @staticmethod
    def params():
        return dict(scope='master', image_size=(160, 160, 4), game_size=1280,
    ...
    def _build_model(self, m={}, estimator=None):
        ...
        reward = self._reward
        estimate_map = self._estimate_map_list  # 多尺度
        ...
        def _estimate(image):
            def _constrain_confidence(belief):
                ...
                res = tf.stack([estimate, tf.nn.sigmoid(confidence)], axis=3)
                # print(res)  # shape=(?, 256, 256, 2) * 3
                return res

            net = image  # shape=(?, 84, 84, 4)
            ...
            with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d],
                    ...
                    for idx, output in enumerate([(8, [7, 7]), (8, [7, 7]), (12, [3, 3]), (12, [3, 3])]):
                        ...
                        # print(net)  ->
                        # shape=(?, 39, 39, 8), shape=(?, 16, 16, 8), shape=(?, 7, 7, 12), shape=(?, 2, 2, 12)
                ...
                with slim.arg_scope([slim.conv2d_transpose],
                    ...
                    for idx, channels in enumerate([16, 2]):
                        ...
                        # print(net)  # shape=(?, 256, 256, 16), shape=(?, 256, 256, 2)
                        last_output_channels = channels

                    beliefs = [self._upscale_image(net, i) for i in xrange(estimate_scale)]
                    # print(beliefs)  # shape=(?, 256, 256, 2) * 3
        ...
        with tf.variable_scope('CMAP', reuse=tf.AUTO_REUSE):
            with tf.variable_scope('batch_norm', reuse=tf.AUTO_REUSE):
                ...
                normalized_space = slim.batch_norm(space_map, is_training=is_training, scope='space')  # ground truth map
            ...
            cmap_cell = CMAPCell()
            inputs = (normalized_input, normalized_space, normalized_goal, egomotion, tf.expand_dims(reward, axis=2))
            # print(inputs)
            # shape=(?, ?, 84, 84, 4), shape=(?, ?, 256, 256, 1), shape=(?, ?, 256, 256, 1), shape=(?, ?, 3), shape=(?, ?, 1)
            results, final_belief = tf.nn.dynamic_rnn(cmap_cell,
                                                      ...
                                                      swap_memory=True,
                                                      scope='network')
            # print(final_belief)  # shape=(?, 256, 256, 3) * 3

        m['estimate_map_list'] = results[:estimate_scale]
        # print(m['estimate_map_list'])  # shape=(?, ?, 256, 256, 3) * 3
        results = results[estimate_scale:]

        m['goal_map_list'] = results[:estimate_scale]
        # print(m['goal_map_list'])  # shape=(?, ?, 256, 256, 1) * 3
        results = results[estimate_scale:]

        m['reward_map_list'] = results[:estimate_scale]
        # print(m['reward_map_list'])  # shape=(?, ?, 16, 16, 1) * 3
        results = results[estimate_scale:]

        m['value_map_list'] = results[:estimate_scale]
        # print(m['value_map_list'])  # shape=(?, ?, 16, 16, 1)
        results = results[estimate_scale:]

        m['action_map_list'] = results[:estimate_scale]
        # print(m['action_map_list'])  # shape=(?, ?, 16, 16, 8)
        results = results[estimate_scale:]

        m['predictions'] = results[0]
        # print(m['predictions'])  # shape=(?, ?, 4)
        results = results[1:]
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, v in self.params().iteritems():
            ...
            # setattr(self, '_{}'.format(k), v)  # just use default params

        with tf.variable_scope("CMPA"):
            self._is_training = tf.placeholder(tf.bool, name='is_training')  
            self._sequence_length = tf.placeholder(tf.int32, [None], name='sequence_length')  # RNN序列长度
            self._visual_input = tf.placeholder(tf.float32, [None, None] + list(self._image_size),  # 图像数据输入
                                                name='visual_input')  # shape=(?, ?, 84, 84, 4)
            self._egomotion = tf.placeholder(tf.float32, (None, None, 3), name='egomotion')  # 自运动信息
            self._reward = tf.placeholder(tf.float32, (None, None), name='reward')  # reward map
            # 空间中free space(depth map), ground_truth
            self._space_map = tf.placeholder(tf.float32, (None, None, self._estimate_size, self._estimate_size, 1),
                                             name='space_map')
            # print(self._space_map)  # shape=(?, ?, 256, 256, 1)
            # 空间中目标位置
            self._goal_map = tf.placeholder(tf.float32, (None, None, self._estimate_size, self._estimate_size, 1),
                                            name='goal_map')
            # print(self._goal_map)  # shape=(?, ?, 256, 256, 1)
            # 以自我为中心的3个尺度的地图
            ...
            # print(self._estimate_map_list)  # shape=(?, 256, 256, 3) * 3
            self._optimal_action = tf.placeholder(tf.int32, (None, None), name='optimal_action')

            tensors = {}
            logits = self._build_model(tensors)  #  shape=(?, 4)
            ...
            # (batch x 1)
            action_mean = tf.reduce_mean(action_counts, 1, keepdims=True)
    ...
    def input_tensors(self):
        return {
            ...
            'egomotion': self._egomotion,
            'reward': self._reward,
            'space_map': self._space_map,  # top view
            'goal_map': self._goal_map,  # top view
            'estimate_map_list': self._estimate_map_list,  # see above (multi-scale)
